#ifndef BST_H
#define BST_H
#include <iostream> #include <exception> #include <cstdlib> #include <utility>
/**
 * A templated class for a Node in a search tree.
 * The getters for parent/left/right are virtual so
 * that they can be overridden for future kinds of
 * search trees, such as Red Black trees, Splay trees, * and AVL trees.
 */
 ...
 * are only used as references to existing nodes. The nodes pointed to by parent/left/right * are freed by the BinarySearchTree.
 */
template <typename Key, typename Value>
Node<Key, Value>::~Node()
{
...
static Node<Key, Value> *predecessor(Node<Key, Value> *current); // TODO // Note: static means these functions don't have a "this" pointer
...
/**
 * An insert method to insert into a Binary Search Tree. * The tree will not remain balanced when inserting.
 * Recall: If key is already in the tree, you should
 * overwrite the current value with the updated value. */
template <class Key, class Value>
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value> &keyValuePair)
{
    // checking if insert is null if (root_ == nullptr)
    {
    ...
    {
        // creating temp node Node<Key, Value> *c = root_;
        // iterating through while true while (true)
        {
            // checking if c key is greater
            if (keyValuePair.first < c->getKey())
            {
                // if c getLeft is null, then we add a new node if (c->getLeft() == NULL)
                {
                    Node<Key, Value> *ins2 = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, c);
                    c->setLeft(ins2);
                    break;
                }
                // if not then we iterate to next left else
                {
                    c = c->getLeft();
                }
            }
            ...
            {
                // if c getRight is null, then we add a new node if (c->getRight() == NULL)
                {
...
/**
 * A remove method to remove a specific key from a Binary Search Tree. * Recall: The writeup specifies that if a node has 2 children you
...
void BinarySearchTree<Key, Value>::remove(const Key &key)
{
    ...
    // if c is null then we return if (c == NULL) {
    return;
}
// case for when there are 2 children
// having this first bc after swap we will either be in a 0-child or 1-child case if(c->getLeft() != NULL && c->getRight() != NULL) {
nodeSwap(c, predecessor(c));
}
// case for when there is one child and 0 children // creating temp node to store it
Node<Key, Value> *one;
// 0 children
...
// updating the parent node to complete the promote functionality Node <Key, Value> * two = c -> getParent();
if (two == NULL)
{
    root_ = one;
}
...
// check to set the parent of one to two if (one != NULL) {
one->setParent(two);
}
// deleting c
delete c;
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value> *current)
{
    ...
    {
        // if it does then we go all the way to the right Node<Key, Value> *cur = current->getLeft();
    ...
    // if not, then we walk up the chain and find the first node // who is a right child of his parent
...
{
    // check if right child exists if (current->getRight() != NULL) {
    // if it does, we go all the way to the left Node<Key, Value> *cur = current->getRight(); while (cur->getLeft() != NULL)
    {
        cur = cur->getLeft();
    }
    return cur;
...
// who is a left child of his parent Node<Key, Value> *cur = current;
while (cur->getParent() != nullptr)
{
...
/**
 * A method to remove all contents of the tree and * reset the values in the tree for use again.
 */
template <typename Key, typename Value>
...
{
    // deleting everything by using post order recursion if (c == NULL)
    {
        return;
    }
...
{
    // iterating all the way to the left Node<Key, Value> *c = root_;
...
/**
 * Helper * return * exists */
function to find a node with given key, k and
                                                a pointer to it or
                                            NULL if no item with that key template <typename Key, typename Value>
                                            Node<Key, Value> *BinarySearchTree<Key, Value>::internalFind(const Key &key) const
{
    // if root is null or if the key is equal if (root_ == NULL)
    {
        return root_;
    }
    ...
    {
        // iterating through the tree to find the node Node<Key, Value> *c = root_;
        while (c != NULL)
        {
            if (c->getKey() == key)
            {
...
{
    // checking if parameter is NULL if (parameter == NULL)
    {
        return 0;
    }
    // calculating height
    ...
    int one = 1;
    // returning valid outputs for height values if (l == (-1))
    {
        return -1;
    }
    if (r == (-1))
